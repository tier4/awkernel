...
// original file path: localization/autoware_gyro_odometer/src/gyro_odometer_core.cpp
// test code: localization/autoware_gyro_odometer/test/test_gyro_odometer_helper.cpp
// version: 1.8.0
...
impl GyroOdometerCore {
    ...
    pub fn process_imu_with_covariance(&mut self, imu: ImuWithCovariance) -> Result<()> {
        self.add_imu(imu);
        Ok(())
    }
...
mod tests {
    ...
    // Equivalent helpers to Autoware C++ test helper.
    fn generate_sample_imu() -> ImuMsg {
        ImuMsg {
            header: Header {
                frame_id: "base_link",
                timestamp: 123456789,
    ...
    fn generate_sample_velocity() -> TwistWithCovarianceStamped {
        TwistWithCovarianceStamped {
            header: Header {
                frame_id: "base_link",
                timestamp: 123456789,
    ...
    fn test_imu_with_covariance_conversion() {
        let imu_msg = generate_sample_imu();
