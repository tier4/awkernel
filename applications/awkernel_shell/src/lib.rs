...
fn perf_ffi() {
    ...
    console::print(" cpu | Type           |   kernel_time  |    task_time   |    idle_time   | interrupt_time |   ctx_switch   | ctx_switch_main|    perf_time   \r\n");
    console::print("=====|================|================|================|================|================|================|================|================\r\n");
    ...
    for cpu_id in 0..awkernel_lib::cpu::num_cpu() {
        ...
        let ctx_switch_time = perf::get_context_switch_time(cpu_id);
        let ctx_switch_main_time = perf::get_context_switch_main_time(cpu_id);
        let perf_time = perf::get_perf_time(cpu_id);

        let msg = format!(
            "{cpu_id:>4} | Total          |{kernel_time:>15} |{task_time:>15} |{idle_time:>15} |{interrupt_time:>15} |{ctx_switch_time:>15} |{ctx_switch_main_time:>15} |{perf_time:>15}\r\n"
        );
        ...
        let ave_ctx_switch_time = perf::get_ave_context_switch_time(cpu_id).unwrap_or(0.0);
        let ave_ctx_switch_main_time =
            perf::get_ave_context_switch_main_time(cpu_id).unwrap_or(0.0);
        let ave_perf_time = perf::get_ave_perf_time(cpu_id).unwrap_or(0.0);

        let msg_ave = format!(
            "     | Avg            | {ave_kernel_time:>14.2} | {ave_task_time:>14.2} |{ave_idle_time:>15.2} |{ave_interrupt_time:>15.2} |{ave_ctx_switch_time:>15.2} |{ave_ctx_switch_main_time:>15.2} |{ave_perf_time:>15.2}\r\n",
        );
        ...
        let worst_ctx_switch_time = perf::get_context_switch_wcet(cpu_id);
        let worst_ctx_switch_main_time = perf::get_context_switch_main_wcet(cpu_id);
        let worst_perf_time = perf::get_perf_wcet(cpu_id);

        let msg_worst = format!(
            "     | Worst          | {worst_kernel_time:>14} | {worst_task_time:>14} |{worst_idle_time:>15} |{worst_interrupt_time:>15} |{worst_ctx_switch_time:>15} |{worst_ctx_switch_main_time:>15} |{worst_perf_time:>15}\r\n",
        );
