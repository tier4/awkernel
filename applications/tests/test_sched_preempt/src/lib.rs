...
use awkernel_lib::{cpu::cpu_id, delay::{wait_microsec, uptime}};
...
pub async fn run() { // TASK ID:1
    wait_microsec(100000);
    log::info!("[{}] Start test_sched_preempt at cpu_id: {}", uptime(), cpu_id());
    ...
    log::info!("[{}] End test_sched_preempt at cpu_id: {}", uptime(), cpu_id());
}
...
async fn check_lowest_task(){
    ...
    if let Some((task_id, priority_info)) = get_lowest_priority_task_info() {
        log::debug!("Task ID: {}, sched_priority: {:?}, task_priority: {:?}, cpu_id: {}", task_id, priority_info.scheduler_priority, priority_info.task_priority, priority_info.cpu_id);
    }
}