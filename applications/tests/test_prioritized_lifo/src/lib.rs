...
const EXPECTED_FIRST: u32 = NUM_WORKERS - 1;

static FIRST_STARTED: AtomicU32 = AtomicU32::new(u32::MAX);
static COMPLETED: AtomicU32 = AtomicU32::new(0);

/// This test confirms LIFO behavior for tasks with the same priority.
///
...
pub async fn run() {
    ...
    spawn(
        "lifo_order_controller".into(),
        async move {
            for id in 0..NUM_WORKERS {
                spawn(
                    format!("lifo_worker_{id}").into(),
                    async move {
                        ...
                        if done == NUM_WORKERS {
                            ...
                            log::debug!(
                                "LIFO first_started={first}, expected={EXPECTED_FIRST}, workers={NUM_WORKERS}"
                            );
        ...
        SchedulerType::PrioritizedLIFO(31),
    )
    .await;
}
