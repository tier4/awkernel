...
const LARGE_BLOCK_SIZE: usize = 32768;    // 32KB - large transfer (within MAXPHYS)
const MEGA_BLOCK_SIZE: usize = 65536;     // 64KB - maximum single transfer size (MAXPHYS limit)
...
pub async fn run() -> Result<(), alloc::borrow::Cow<'static, str>> {
    ...
    info!("Using device {} for load testing (block size: {} bytes)", 
...
async fn test_sequential_reads(device_id: u64, block_size: usize) -> bool {
    ...
    for (size, desc) in &test_sizes {
        ...
        info!("    {} reads: {:.2} MB/s, {:.0} IOPS, {:.2}ms avg latency",
            desc,
...
async fn test_sequential_writes(device_id: u64, block_size: usize) -> bool {
    ...
    for (size, desc) in &test_sizes {
        ...
        info!("    {} writes: {:.2} MB/s, {:.0} IOPS, {:.2}ms avg latency",
            desc,
            stats.throughput_mbps(),
            stats.iops(),
            stats.avg_latency_ms()
...
async fn test_random_access(device_id: u64, block_size: usize) -> bool {
    ...
    info!("  Random access: {:.2} MB/s, {:.0} IOPS, {:.2}ms avg latency",
        stats.throughput_mbps(),
        stats.iops(),
        stats.avg_latency_ms()
    );
...
async fn test_mixed_workload(device_id: u64, block_size: usize) -> bool {
    ...
    info!("  Mixed workload: {:.2} MB/s, {:.0} IOPS, {:.2}ms avg latency",
        stats.throughput_mbps(),
        stats.iops(),
        stats.avg_latency_ms()
    );
...
async fn concurrent_io_task(device_id: u64, block_size: usize, task_id: usize, num_ops: usize) -> Result<(), Cow<'static, str>> {
    let mut buffer = vec![0u8; block_size];
    let base_lba = 10000u64 + (task_id as u64 * 100); // Each task gets its own LBA range (starting at 10000 to avoid conflicts)
...
async fn test_large_blocks(device_id: u64, block_size: usize) -> bool {
    ...
    let safe_lba = 20000u64; // Use high LBA to avoid conflicts with other tests
            ...
            Ok(()) => {
                ...
                for (j, (expected, actual)) in buffer.iter().zip(read_buffer.iter()).enumerate() {
                    if expected != actual {
                        mismatches += 1;
                        if mismatches <= 5 {
                            error!("    Data mismatch at offset {j}: expected 0x{expected:02x}, got 0x{actual:02x}");
                        }
                    }
                }
                if mismatches > 0 {
                    error!("    Total {mismatches} bytes mismatched");
                    stats.errors += 1;
                    TOTAL_ERRORS.fetch_add(1, Ordering::Relaxed);
                }
            }
    ...
    info!("  Large blocks (64KB): {:.2} MB/s, {:.0} IOPS, {:.2}ms avg latency",
        stats.throughput_mbps(),
        stats.iops(),
        stats.avg_latency_ms()
    );
...
async fn test_sustained_load(device_id: u64, block_size: usize) -> bool {
    ...
    let blocks = MEDIUM_BLOCK_SIZE / block_size;
    let safe_lba = 30000u64; // Use even higher LBA to avoid all conflicts
    ...
    if stats.errors > 0 {
        error!("    {} errors occurred ({:.2}% error rate)", 
            stats.errors,
