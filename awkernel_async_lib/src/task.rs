...
pub mod perf {
    ...
    static SEND_INNER_TIMESTAMP: Mutex<Option<[u64; MAX_LOGS]>> = Mutex::new(None);
    static RECV_INNER_TIMESTAMP: Mutex<Option<[u64; MAX_LOGS]>> = Mutex::new(None);
    ...
    pub fn update_pre_send_inner_timestamp_at(index: usize, new_timestamp: u64) {
        ...
        let mut node = MCSNode::new();
        let mut recorder_opt = SEND_INNER_TIMESTAMP.lock(&mut node);
    ...
    pub fn update_fin_recv_inner_timestamp_at(index: usize, new_timestamp: u64) {
        ...
        let mut node = MCSNode::new();
        let mut recorder_opt = RECV_INNER_TIMESTAMP.lock(&mut node);
    ...
    pub fn print_timestamp_table() {
        ...
        let send_inner_opt = SEND_INNER_TIMESTAMP.lock(&mut node1);
        let recv_inner_opt = RECV_INNER_TIMESTAMP.lock(&mut node2);
        ...
        for i in 0..MAX_LOGS {
            let pre_send_outer = send_inner_opt.as_ref().map_or(0, |arr| arr[i]);
            let fin_recv_outer = recv_inner_opt.as_ref().map_or(0, |arr| arr[i]);

            if pre_send_outer != 0 || fin_recv_outer != 0 {
