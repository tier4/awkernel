    ...
    fn wake_task(&self, task: Arc<Task>) {
        let (wake_time, absolute_deadline, partitioned_core) = {
            ...
            match info.scheduler_type {
                SchedulerType::PartitionedGEDF(relative_deadline, paritioned_core) => {
                    ...
                    (wake_time, absolute_deadline, paritioned_core)
                }
                _ => unreachable!(),
            }
        };
...
fn calculate_and_update_dag_deadline(dag_info: &DagInfo, wake_time: u64) -> u64 {
