...
impl FileSystem for OverlayFS {
    ...
    fn open_file(&self, path: &str) -> VfsResult<Box<dyn SeekAndRead<Error = VfsError> + Send>> {
        self.read_path(path)?.open_file()
    }

    fn create_file(&self, path: &str) -> VfsResult<Box<dyn SeekAndWrite<Error = VfsError> + Send>> {
        self.ensure_has_parent(path)?;
    ...
    fn append_file(&self, path: &str) -> VfsResult<Box<dyn SeekAndWrite<Error = VfsError> + Send>> {
