...
use super::{error::VfsError, error::VfsResult, filesystem::FileSystem};
...
impl<E: IoError> PartialEq for VfsPath<E> {
    fn eq(&self, other: &Self) -> bool {
        self.path == other.path && Arc::ptr_eq(&self.fs, &other.fs)
    }
}

impl<E: IoError> Eq for VfsPath<E> {}

impl<E: IoError + Clone + 'static> VfsPath<E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn create_dir(&self) -> VfsResult<(), E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn create_dir_all(&self) -> VfsResult<(), E> {
        let mut pos = 1;
        let path = &self.path;
        if path.is_empty() {
            // root exists always
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn read_dir(&self) -> VfsResult<Box<dyn Iterator<Item = VfsPath<E>> + Send>, E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn create_file(&self) -> VfsResult<Box<dyn SeekAndWrite<Error = VfsError<E>> + Send>, E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn open_file(&self) -> VfsResult<Box<dyn SeekAndRead<Error = VfsError<E>> + Send>, E> {
    ...
    fn get_parent(&self, action: &str) -> VfsResult<(), E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn append_file(&self) -> VfsResult<Box<dyn SeekAndWrite<Error = VfsError<E>> + Send>, E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn remove_file(&self) -> VfsResult<(), E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn remove_dir(&self) -> VfsResult<(), E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn remove_dir_all(&self) -> VfsResult<(), E> {
        if !self.exists()? {
            return Ok(());
        }
    ...
    /// # Ok::<(), VfsError>(())
    pub fn metadata(&self) -> VfsResult<VfsMetadata, E> {
    ...
    ///
    /// # Ok::<(), VfsError>(())
    pub fn set_creation_time(&self, time: Time) -> VfsResult<(), E> {
        self.fs
            .fs
    ...
    ///
    /// # Ok::<(), VfsError>(())
    pub fn set_modification_time(&self, time: Time) -> VfsResult<(), E> {
        self.fs
            .fs
    ...
    ///
    /// # Ok::<(), VfsError>(())
    pub fn set_access_time(&self, time: Time) -> VfsResult<(), E> {
    ...
    /// assert!(file.is_file()?);
    /// # Ok::<(), VfsError>(())
    pub fn is_file(&self) -> VfsResult<bool, E> {
        if !self.exists()? {
            return Ok(false);
        }
    ...
    /// assert!(!file.is_dir()?);
    /// # Ok::<(), VfsError>(())
    pub fn is_dir(&self) -> VfsResult<bool, E> {
        if !self.exists()? {
            return Ok(false);
        }
        let metadata = self.metadata()?;
    ...
    /// assert!(directory.exists()?);
    /// # Ok::<(), VfsError>(())
    pub fn exists(&self) -> VfsResult<bool, E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn walk_dir(&self) -> VfsResult<WalkDirIterator<E>, E> {
        Ok(WalkDirIterator {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn read_to_string(&self) -> VfsResult<String, E> {
        let metadata = self.metadata()?;
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn copy_file(&self, destination: &VfsPath<E>) -> VfsResult<(), E> {
        || -> VfsResult<(), E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn move_file(&self, destination: &VfsPath<E>) -> VfsResult<(), E> {
        || -> VfsResult<(), E> {
            if destination.exists()? {
                return Err(VfsError::from(VfsErrorKind::Other(
                    "Destination exists already".into(),
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn copy_dir(&self, destination: &VfsPath<E>) -> VfsResult<u64, E> {
    ...
    /// # Ok::<(), VfsError>(())
    /// ```
    pub fn move_dir(&self, destination: &VfsPath<E>) -> VfsResult<(), E> {
        || -> VfsResult<(), E> {
            if destination.exists()? {
                return Err(VfsError::from(VfsErrorKind::Other(
                    "Destination exists already".into(),
...
pub struct WalkDirIterator<E: IoError> {
    /// the path iterator of the current directory
    inner: Box<dyn Iterator<Item = VfsPath<E>> + Send>,
    /// stack of subdirectories still to walk
    todo: Vec<VfsPath<E>>,
}

impl<E: core::fmt::Debug + IoError> core::fmt::Debug for WalkDirIterator<E> {
...
impl<E: IoError + Clone + 'static> Iterator for WalkDirIterator<E> {
    type Item = VfsResult<VfsPath<E>, E>;
    ...
    W: Write + ?Sized,
{
    ...
    loop {
        ...
        match writer.write_all(&buf[..bytes_read]) {
            Ok(_) => {}
            Err(ref e) if e.is_interrupted() => {
                // Since is_interrupted will be handled in write_all().
                unreachable!();
            }
            Err(e) => {
                return Err(CopyError::WriteError(e));
            }
        }
