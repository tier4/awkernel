...
const POOL_NOT_INITIALIZED: &str = "Storage transfer pool not initialized. Call init_transfer_pool() first";
...
// Safe transfer access functions - no unsafe code needed!
...
pub fn transfer_is_completed(id: u16) -> Result<bool, StorageManagerError> {
    ...
    Ok(pool.transfers[id as usize].completed.load(Ordering::Acquire))
}
...
pub fn add_storage_device(device: Arc<dyn StorageDevice>, namespace_id: Option<u32>) -> u64 {
        ...
        if completed {
            ...
            if is_poll {
                ...
                if self.poll_count > timeout_ms {
                    ...
                    return Poll::Ready(Err(StorageManagerError::DeviceError(StorageDevError::IoError)));
                }
            }
