use super::page_allocator::{get_page_table, HeapPageAllocator};
use crate::{
...
impl crate::paging::Mapper for super::X86 {
    unsafe fn map(
        ...
        let mut page_table = get_page_table().ok_or(MapError::InvalidPageTable)?;

        let page = Page::<Size4KiB>::from_start_address(VirtAddr::new(vm_addr as u64))
            .or(Err(MapError::AddressNotAligned))?;

        let phy_frame = PhysFrame::from_start_address(PhysAddr::new(phy_addr as u64))
            .or(Err(MapError::AddressNotAligned))?;
        ...
        let flusher = page_table
            .map_to(page, phy_frame, f, &mut allocator)
