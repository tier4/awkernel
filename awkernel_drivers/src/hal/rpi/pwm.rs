...
impl Pwm {
    pub fn new() -> Result<Pwm, &'static str> {
        let pin18 = GpioPin::new(18)?;
        let pin18 = pin18.into_alt(GpioFunction::ALTF5, PullMode::None)?;

        // let pin19 = GpioPin::new(19)?;
        // let pin19 = pin19.into_alt(GpioFunction::ALTF5, PullMode::None)?;
    ...
    pub fn enable(&mut self) -> Result<(), PwmError> {
        // Clear FIFO
        registers::CTL.write(registers::Control::CLRF, self.base);
        registers::CTL.write(
            registers::Control::PWEN1 | registers::Control::PWEN2,
            self.base,
        );
        Ok(())
    }
