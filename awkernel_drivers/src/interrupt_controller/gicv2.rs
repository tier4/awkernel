...
mod registers {
    ...
    bitflags! {
        ...
        pub struct GiccCtrlNonSecure: u32 {
...
impl GICv2 {
    pub fn new(gicd_base: usize, gicc_base: usize) -> Self {
        ...
        registers::GICD_CTRL.write(registers::GicdCtrlNonSecure::empty(), gicd_base);
        registers::GICC_CTRL.write(registers::GiccCtrlNonSecure::empty(), gicc_base);
        ...
        registers::GICD_CTRL.write(registers::GicdCtrlNonSecure::ENABLE, gicd_base);

        // Mask interrupts whose priority is greater than 0x80.
        registers::GICC_PMR.write(0xF0, gicc_base);

        // Enable the CPU interface.
        registers::GICC_CTRL.write(registers::GiccCtrlNonSecure::ENABLE_GRP1, gicc_base);
    ...
    fn probe_max_it(&mut self) {
        ...
        let old_ctrl = registers::GICC_CTRL.read(self.gicc_base);
        registers::GICC_CTRL.write(registers::GiccCtrlNonSecure::empty(), self.gicc_base);
        ...
        for i in (0..=max_regs).rev() {
            ...
            for b in (0..NUM_INTS_PER_REG).rev() {
                if (1 << b) & reg > 0 {
                    ...
                    registers::GICC_CTRL.write(old_ctrl, self.gicc_base);
                    return;
                }
            }
        }
...
impl InterruptController for GICv2 {
    ...
    fn init_non_primary(&mut self) {
        ...
        // Enable the CPU interface.
        registers::GICC_CTRL.write(registers::GiccCtrlNonSecure::ENABLE_GRP1, self.gicc_base);
    }
