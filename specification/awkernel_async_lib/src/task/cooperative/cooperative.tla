...
begin
    start_worker:
        if self = 1 then
            call wake_task_all(TASK_NUM \div 2);
        end if;
...
\* BEGIN TRANSLATION (chksum(pcal) = "1279f3f4" /\ chksum(tla) = "e642418f")
\* Procedure variable task of procedure run_main at line 101 col 5 changed to task_
\* Parameter task of procedure wake_task at line 36 col 21 changed to task_w
\* Parameter pid of procedure get_next at line 63 col 20 changed to pid_
\* Parameter pid of procedure run_main at line 99 col 20 changed to pid_r
\* Parameter task of procedure future at line 166 col 23 changed to task_f
CONSTANT defaultInitValue
VARIABLES queue, lock_info, lock_future, lock_scheduler, in_queue, state, 
          result_next, result_future, wake_other, pc, stack
...
VARIABLES task_w, pid_, head, pid_r, task_, pid, task_f, task

vars == << queue, lock_info, lock_future, lock_scheduler, in_queue, state, 
           result_next, result_future, wake_other, pc, stack, task_w, pid_, 
...
check_wake_task(self) == /\ pc[self] = "check_wake_task"
                         /\ IF state[task_w[self]] = "Terminated"
                               THEN /\ lock_info' = [lock_info EXCEPT ![task_w[self]] = FALSE]
                                    /\ lock_scheduler' = FALSE
                                    /\ pc' = [pc EXCEPT ![self] = Head(stack[self]).pc]
                                    /\ task_w' = [task_w EXCEPT ![self] = Head(stack[self]).task_w]
                                    /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
                               ELSE /\ pc' = [pc EXCEPT ![self] = "set_inq_wake_task"]
                                    /\ UNCHANGED << lock_info, lock_scheduler, 
                                                    stack, task_w >>
                         /\ UNCHANGED << queue, lock_future, in_queue, state, 
                                         result_next, result_future, 
                                         wake_other, pid_, head, pid_r, task_, 
                                         pid, task_f, task >>
...
end_wake_task(self) == /\ pc[self] = "end_wake_task"
                       ...
                       /\ task_w' = [task_w EXCEPT ![self] = Head(stack[self]).task_w]
                       /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
                       /\ UNCHANGED << lock_info, lock_future, in_queue, state, 
...
wake_task(self) == start_wake_task(self) \/ wait_wake_task(self)
                      \/ check_wake_task(self) \/ set_inq_wake_task(self)
                      \/ end_wake_task(self)
                        ...
                        /\ IF queue = <<>>
                              ...
                              ELSE /\ pc' = [pc EXCEPT ![self] = "pop_get_next"]
                                   /\ UNCHANGED << lock_scheduler, result_next, 
                                                   stack, pid_, head >>
                        /\ UNCHANGED << queue, lock_info, lock_future, 
                                        in_queue, state, result_future, 
                                        wake_other, task_w, pid_r, task_, pid, 
                                        task_f, task >>
...
pop_get_next(self) == /\ pc[self] = "pop_get_next"
                      ...
                      /\ UNCHANGED << lock_info, lock_future, lock_scheduler, 
                                      ...
                                      pid_, pid_r, task_, pid, task_f, task >>
...
end_get_next(self) == /\ pc[self] = "end_get_next"
                      ...
                      /\ pid_' = [pid_ EXCEPT ![self] = Head(stack[self]).pid_]
                      /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
                      /\ UNCHANGED << queue, lock_future, state, result_future, 
                                      wake_other, task_w, pid_r, task_, pid, 
                                      task_f, task >>
...
lock_future_run_main(self) == /\ pc[self] = "lock_future_run_main"
                              /\ IF lock_future[task_[self]]
                                    ...
                                    ELSE /\ lock_future' = [lock_future EXCEPT ![task_[self]] = TRUE]
                                         ...
                                         /\ UNCHANGED << stack, task_w >>
                              /\ UNCHANGED << queue, lock_info, lock_scheduler, 
                                              in_queue, state, result_next, 
                                              result_future, wake_other, pid_, 
                                              head, pid_r, task_, pid, task_f, 
                                              task >>
...
pre_future_run_main(self) == /\ pc[self] = "pre_future_run_main"
                             ...
                             /\ pc' = [pc EXCEPT ![self] = "future_run_main"]
...
post_future_run_main(self) == /\ pc[self] = "post_future_run_main"
                              ...
                              /\ IF result_future[pid_r[self]] = "Pending"
                                    ...
                                    ELSE /\ IF result_future[pid_r[self]] = "Ready"
                                               ...
                                               ELSE /\ Assert((FALSE), 
                                                              "Failure of assertion at line 148, column 13.")
                                                    /\ state' = state
...
unlock_run_main(self) == /\ pc[self] = "unlock_run_main"
                         ...
                         /\ UNCHANGED << queue, lock_scheduler, in_queue, 
                                         state, result_next, result_future, 
                                         wake_other, stack, task_w, pid_, head, 
...
run_main(self) == start_run_main(self) \/ get_task_run_main(self)
                     ...
                     \/ pre_future_run_main(self) \/ future_run_main(self)
                     \/ post_future_run_main(self) \/ unlock_run_main(self)
                      ...
                      /\ IF task_f[self] > TASK_NUM \div 2
                            THEN /\ IF wake_other[task_f[self]]
                                       THEN /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake_task",
                                                                                        ...
                                                                                        task_w    |->  task_w[self] ] >>
                                                                                    \o stack[self]]
                                               /\ task_w' = [task_w EXCEPT ![self] = task_f[self]]
                                            /\ pc' = [pc EXCEPT ![self] = "start_wake_task"]
                                       ELSE /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake_task",
                                                                                        pc        |->  "set_result_future2",
                                                                                        task_w    |->  task_w[self] ] >>
                                                                                    \o stack[self]]
                                               /\ task_w' = [task_w EXCEPT ![self] = task_f[self] - (TASK_NUM \div 2)]
                                            /\ pc' = [pc EXCEPT ![self] = "start_wake_task"]
                            ELSE /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake_task",
...
end_future(self) == /\ pc[self] = "end_future"
                    ...
                    /\ UNCHANGED << queue, lock_info, lock_future, 
                                    lock_scheduler, in_queue, state, 
                                    result_next, result_future, wake_other, 
                                    task_w, pid_, head, pid_r, task_, task >>
...
start_wake_task_all(self) == /\ pc[self] = "start_wake_task_all"
                             /\ IF task[self] > TASK_NUM
                                   ...
                                   ELSE /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake_task",
                                                                                    ...
                                                                                    task_w    |->  task_w[self] ] >>
                                                                                \o stack[self]]
                                           /\ task_w' = [task_w EXCEPT ![self] = task[self]]
                                        /\ pc' = [pc EXCEPT ![self] = "start_wake_task"]
                                        /\ task' = task
                             /\ UNCHANGED << queue, lock_info, lock_future, 
                                             lock_scheduler, in_queue, state, 
                                             result_next, result_future, 
                                             wake_other, pid_, head, pid_r, 
                                             task_, pid, task_f >>
...
start_worker(self) == /\ pc[self] = "start_worker"
                      /\ IF self = 1
                            THEN /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake_task_all",
                                                                             ...
                                                                             task      |->  task[self] ] >>
                                                                         \o stack[self]]
                                    /\ task' = [task EXCEPT ![self] = TASK_NUM \div 2]
                                 /\ pc' = [pc EXCEPT ![self] = "start_wake_task_all"]
...
run(self) == /\ pc[self] = "run"
             ...
             /\ UNCHANGED << queue, lock_info, lock_future, lock_scheduler, 
                             in_queue, state, result_next, result_future, 
                             wake_other, task_w, pid_, head, pid, task_f, task >>
...
Next == (\E self \in ProcSet:  \/ wake_task(self) \/ get_next(self)
