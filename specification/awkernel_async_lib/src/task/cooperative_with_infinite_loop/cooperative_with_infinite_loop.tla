...
variables
    ...
    in_queue = [x \in 1..TOTAL_TASK_NUM |-> FALSE];
...
\* BEGIN TRANSLATION (chksum(pcal) = "b9143f44" /\ chksum(tla) = "804acd8")
\* Procedure variable task of procedure run_main at line 123 col 5 changed to task_
\* Parameter task of procedure wake_task at line 36 col 21 changed to task_w
\* Parameter task of procedure wake at line 56 col 16 changed to task_wa
\* Parameter pid of procedure get_next at line 84 col 20 changed to pid_
\* Parameter pid of procedure run_main at line 121 col 20 changed to pid_r
\* Parameter task of procedure future at line 199 col 23 changed to task_f
CONSTANT defaultInitValue
VARIABLES queue, lock_info, lock_future, lock_scheduler, in_queue, need_sched, 
          state, result_next, result_future, waked, pc, stack
...
vars == << queue, lock_info, lock_future, lock_scheduler, in_queue, 
           need_sched, state, result_next, result_future, waked, pc, stack, 
...
Init == (* Global variables *)
        ...
        /\ waked = [x \in 1..TOTAL_TASK_NUM |-> FALSE]
        (* Procedure wake_task *)
...
start_wake_task(self) == /\ pc[self] = "start_wake_task"
                         ...
                         /\ UNCHANGED << queue, lock_info, lock_future, 
                                         ...
                                         result_next, result_future, waked, 
...
set_inq_wake_task(self) == /\ pc[self] = "set_inq_wake_task"
                           ...
                           /\ UNCHANGED << queue, lock_future, lock_scheduler, 
                                           ...
                                           result_future, waked, stack, task_w, 
...
end_wake_task(self) == /\ pc[self] = "end_wake_task"
                       ...
                       /\ UNCHANGED << lock_info, lock_future, in_queue, 
                                       need_sched, state, result_next, 
                                       result_future, waked, task_wa, pid_, 
...
need_sched_wake(self) == /\ pc[self] = "need_sched_wake"
                         ...
                         /\ UNCHANGED << queue, lock_future, lock_scheduler, 
                                         ...
                                         result_future, waked, task_w, pid_, 
                                         head, pid_r, task_, pid, task_f, task >>
...
end_wake(self) == /\ pc[self] = "end_wake"
                  ...
                  /\ UNCHANGED << queue, lock_info, lock_future, 
                                  ...
                                  result_next, result_future, waked, task_wa, 
                                  pid_, head, pid_r, task_, pid, task_f, task >>
...
check_get_next(self) == /\ pc[self] = "check_get_next"
                        ...
                        /\ UNCHANGED << queue, lock_info, lock_future, 
                                        in_queue, need_sched, state, 
                                        result_future, waked, task_w, task_wa, 
                                        pid_r, task_, pid, task_f, task >>
...
start_run_main(self) == /\ pc[self] = "start_run_main"
                        ...
                        /\ UNCHANGED << queue, lock_info, lock_future, 
                                        ...
                                        waked, task_w, task_wa, pid_r, task_, 
                                        pid, task_f, task >>
...
start_future(self) == /\ pc[self] = "start_future"
                      /\ IF task_f[self] > INFINITE_TASK_NUM
                            THEN /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake",
                                 ...
                                 /\ pc' = [pc EXCEPT ![self] = "start_wake"]
                            ELSE /\ IF waked[task_f[self] + INFINITE_TASK_NUM]
                                       THEN /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "wake",
                      ...
                      /\ UNCHANGED << queue, lock_info, lock_future, 
                                      lock_scheduler, in_queue, need_sched, 
                                      state, result_next, result_future, waked, 
...
set_result_future3(self) == /\ pc[self] = "set_result_future3"
                            /\ waked' = [waked EXCEPT ![task_f[self] + INFINITE_TASK_NUM] = TRUE]
                            /\ result_future' = [result_future EXCEPT ![pid[self]] = "Pending"]
                            /\ pc' = [pc EXCEPT ![self] = "end_future"]
                            /\ UNCHANGED << queue, lock_info, lock_future, 
                                            lock_scheduler, in_queue, 
...
run(self) == /\ pc[self] = "run"
             ...
             /\ UNCHANGED << queue, lock_info, lock_future, lock_scheduler, 
                             in_queue, need_sched, state, result_next, 
                             result_future, waked, task_w, task_wa, pid_, head, 
                             pid, task_f, task >>
